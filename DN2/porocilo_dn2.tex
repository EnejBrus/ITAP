...
\section*{Naloga 1: Izbira modelov}

\subsection*{1.a Identifikacija modelov po odločitvenih mejah}
\begin{itemize}
    \item \textbf{Model A:}
    \begin{itemize}
        \item Ime modela: KNN (najverjetneje z majhnim \texttt{k}, npr. \texttt{k=1}).
        \item Zakaj si model izbral: Meja je zelo nelinearna in lokalno ``razcefrana'' z majhnimi otočki. To je tipično za KNN, ko napoved močno določajo najbližji posamezni primeri.
...
\newpage
    \item \textbf{Model D:}
    \begin{itemize}
        \item Ime modela: Logistična regresija (alternativno tudi linearni SVM s kalibracijo verjetnosti).
        \item Zakaj si model izbral: Meja je skoraj linearna, verjetnost pa se gladko spreminja po prostoru, kar je značilno za linearne klasifikatorje.
        \item Kako bi ga izboljšal: Dodal bi nelinearne/interakcijske značilke in prilagodil regularizacijo (\texttt{C}). Pri SVM bi testiral tudi RBF jedro.
        \item Kako bi to vplivalo na krivuljo: Pri linearnem modelu bi meja ostala pretežno ravna in stabilna, z nelinearnimi značilkami ali RBF jedrom pa bi postala bolj ukrivljena in bi bolje sledila strukturi podatkov.
    \end{itemize}
\end{itemize}

\subsection*{1.b Analiza štirih scenarijev}
...
\section*{Naloga 2: Vrečenje z naključnimi podprostori}
...
\subsection*{2.a Bootstrap vzorčenje}
...
\subsection*{2.b Naključni podprostor značilk}
...
\subsection*{2.c Razred VrecaModelov}
...
\subsection*{2.d Primerjava konfiguracij}
...
\subsection*{2.e Napovedna moč spremenljivk}
...
\section*{Naloga 3: Klasifikacija oblačil s slikovnimi podatki}

\subsection*{Kaj je naloga zahtevala in kako smo se je lotili}
...
\subsection*{Izbira modelov: zakaj Random Forest in CNN}
Izbrali smo dva ključna modela:
\begin{itemize}
    \item \textbf{Random Forest} kot ne-nevronski baseline,
...
\subsection*{Kako smo modela implementirali}
...
\subsection*{Rezultati in stabilnost ocene}
...
\subsection*{Kateri model je boljši in zakaj}
